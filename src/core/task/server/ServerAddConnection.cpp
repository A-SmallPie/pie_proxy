...
void ServerAddConnection::run(ServerThreadResource& res){
    ...
    std::shared_ptr<ServerConnection> connection = std::make_shared<ServerConnection>(client_socket_, client_ip_, RECV_BUFFER_SIZE, std::move(event), res);
