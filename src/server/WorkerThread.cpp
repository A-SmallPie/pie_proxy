...
:ID_(id){
    resource_=std::make_unique<ServerThreadResource>(id, max_event);
}
...
void WorkerThread::modify_epoll_events(ServerConnection* connection, uint32_t op){
    // 处理各种事务， 暂不实现
    connection->get_ip();
