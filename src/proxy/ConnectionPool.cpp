...
#include "proxy/ConnectionPool.hpp"
#include <mutex>
...
:INITIAL_POOL_CAPACITY(initial_pool_capacity), MAX_POOL_CAPACITY(max_pool_capacity){
    for(int i=0; i<INITIAL_POOL_CAPACITY; i++){
        ServerConnection* connection = new ServerConnection(1024);
        pool_.emplace(connection);
    }
}

bool ConnectionPool::acquire(int client_socket, std::string client_ip, ServerConnection** connection){
...
void ConnectionPool::scale_out(){
    ...
    for(size_t i=0; i<current_pool_capacity_-capacity; i++){
        ServerConnection* connection = pool_.front();
        pool_.pop();
        delete(connection);
    }
    current_pool_capacity_ = capacity;
...
void ConnectionPool::release(ServerConnection* connection){
    connection->reset();
    std::lock_guard<std::mutex> lock(mutex_);
    pool_.emplace(connection);
}
